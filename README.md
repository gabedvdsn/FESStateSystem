...
If either `StateConditionalTrigger` is left as `null`, it will always activate to `True`. Some responders will never activate based on the conditionals. For example, detecting a change FROM a state in *priority 0* TO a state in *priority 1* could never activate the responder. Likewise, detecting a change FROM **State A** TO **State A** could never activate the trigger, unless the `StateTrigger` which activated the transition has `ReEnterSameStates` set to `True`.
